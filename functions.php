...
function comment_mail_notify($comment_id) {
    ...
    if (($parent_id != '') && ($spam_confirmed != 'spam')) {
        $wp_email = 'me@' . preg_replace('#^www.#', '', strtolower($_SERVER['SERVER_NAME'])); //e-mail 发出点, no-reply 可改为可用的 e-mail.
        $to = trim(get_comment($parent_id)->comment_author_email);
        $subject = 'Hi! 您在 [' . get_option("blogname") . '] 的评论有了回复';
        $message = '
                            ...
                            <td align="center">
                                <table class="logo" style="margin-bottom: 10px;">
                                    <tbody>
                                        <tr>
                                            <td>
<span style="font-size: 22px;padding: 10px 20px;margin-bottom: 5%;color: #65c5ff;border: 1px solid;border-radius: 2px;display: inline-block;">' . get_option("blogname") . '</span>
                        ...
                        <tr class="content">
                            <td>
                                ...
                                <p>Hi, ' . trim(get_comment($parent_id)->comment_author) . '!
                                    <br>你在文章 "' . get_the_title($comment->comment_post_ID) . '" 下的评论：</p>
                                <p style="background: #eee;padding: 1em;text-indent: 2em;line-height: 30px;">' . trim(get_comment($parent_id)->comment_content) . '</p>
                                <p>'. $comment->comment_author .' 给你了回复：</p>
...
<a target="_blank" style="color: #fff;background: #65c5ff;box-shadow: 0 5px 20px -10px #44b0f1;border: 1px solid #44b0f1;width: 200px;font-size: 14px;padding: 10px 0;border-radius: 2px;margin: 10% 0 5%;text-align:center;display: inline-block;text-decoration: none;"
                                                href="' . htmlspecialchars(get_comment_link($parent_id)) . '">查看</a>
...
<table border="0" cellpadding="0" cellspacing="0" align="center" class="footer" style="max-width: 550px; font-family: Lato, \'Lucida Sans\', \'Lucida Grande\', SegoeUI, \'Helvetica Neue\', Helvetica, Arial, sans-serif; font-size: 15px; line-height: 22px; color: #444444; text-align: left; padding: 20px 0; font-weight: normal;">
    <tbody>
        <tr>
            <td align="center" style="text-align: center; font-size: 12px; line-height: 18px; color: rgb(163, 163, 163); padding: 5px 0px;"></td>
        </tr>
        <tr>
            <td style="text-align: center; font-weight: normal; font-size: 12px; line-height: 18px; color: rgb(163, 163, 163); padding: 5px 0px;">
                ...
                <p>© '.date("Y").' <a name="footer_copyright" href="' . home_url() . '" target="_blank">' . get_option("blogname") . '</a> / Powered by <a href="https://wordpress.org">WordPress</a> / Plugin by
                    <a
                    href="https://wpmailsmtp.com/">WP Mail SMTP</a>/ Theme by <a href="https://biji.io/">Adams</a>
                </p>
            </td>
        </tr>
    </tbody>
